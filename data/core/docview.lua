...
DocView.translate = {
    ...
    ["next_page"] = function(doc, line, col, dv)
        if line == doc.buffer:line_count() then
            return doc.buffer:line_count(), #doc.buffer:get_line(line)
        end
        local min, max = dv:get_visible_line_range()
        return line + (max - min), 1
    end,
    ...
    ["next_line"] = function(doc, line, col, dv)
        if line == doc.buffer:line_count() then
            return doc.buffer:line_count(), math.huge
        end
        return move_to_line_offset(dv, line, col, 1)
    end
}
...
function DocView:get_scrollable_size()
    if not config.scroll_past_end then
        ...
        return self:get_line_height() * (self.doc.buffer:line_count()) + style.padding.y * 2 + h_scroll
    end
    return self:get_line_height() * (self.doc.buffer:line_count() - 1) + self.size.y
end
...
function DocView:get_gutter_width()
    ...
    return self:get_font():get_width(self.doc.buffer:line_count()) + padding, padding
end
...
function DocView:get_visible_line_range()
    ...
    local maxline = math.min(self.doc.buffer:line_count(), math.floor((y2 - style.padding.y) / lh) + 1)
...
function DocView:get_x_offset_col(line, x)
    local line_text = self.doc.buffer:get_line(line)
...
function DocView:resolve_screen_position(x, y)
    ...
    line = common.clamp(line, 1, self.doc.buffer:line_count())
...
function DocView:scroll_to_make_visible(line, col)
    ...
    local target_y = common.clamp(self.scroll.to.y, ly - oy - self.size.y + scroll_h + overscroll, ly - oy - lh)
    local diff_y = math.abs(target_y - self.scroll.to.y)
    ...
    local target_x = self.scroll.to.x
    if xsup > self.scroll.x + size_x then
        target_x = xsup - size_x
    elseif xinf < self.scroll.x then
        target_x = math.max(0, xinf)
    end
    local diff_x = math.abs(target_x - self.scroll.to.x)
    self.scroll.to.x = target_x

    -- prevent smooth scrolling for < 2 lines to prevent visual glitching
    -- this happens for example when holding down the enter key or arrows
    if diff_y < lh * 2 then
        self.scroll.y = self.scroll.to.y
    end
    if diff_x < lh * 2 then
...
function DocView:on_mouse_moved(x, y, ...)
    ...
    if self.mouse_selecting then
        ...
        if keymap.modkeys["ctrl"] then
            ...
            self.doc.selections = {}
            for i = l1, l2 do
                self.doc:set_selections(i - l1 + 1, i, math.min(c1, #self.doc.buffer:get_line(i)), i,
                    math.min(c2, #self.doc.buffer:get_line(i)))
            end
        else
            if snap_type then
...
function DocView:mouse_selection(doc, snap_type, current_line, current_col, start_line, start_col)
    local swap = start_line < current_line or start_line == current_line and start_col <= current_col
    if swap then
        current_line, current_col, start_line, start_col = start_line, start_col, current_line, current_col
    end
    if snap_type == "word" then
        current_line, current_col = translate.start_of_word(doc, current_line, current_col)
        start_line, start_col = translate.end_of_word(doc, start_line, start_col)
    elseif snap_type == "lines" then
        current_col = 1
        if start_line >= doc.buffer:line_count() then
            start_col = math.huge
        else
            start_col = 1
    ...
    if swap then
        return start_line, start_col, current_line, current_col
    end
    return current_line, current_col, start_line, start_col
end
...
function DocView:on_mouse_pressed(button, x, y, clicks)
    ...
    if keymap.modkeys["shift"] then
        ...
        if line > sline then
            self.doc:set_selection(sline, 1, line, #self.doc.buffer:get_line(line))
        else
            self.doc:set_selection(line, 1, sline2, #self.doc.buffer:get_line(sline2))
        end
    else
        if clicks == 1 then
            self.doc:set_selection(line, 1, line, 1)
        elseif clicks == 2 then
            self.doc:set_selection(line, 1, line, #self.doc.buffer:get_line(line))
        end
    end
    return true
end
...
function DocView:update()
    ...
    if (line1 ~= self.last_line1 or col1 ~= self.last_col1 or line2 ~= self.last_line2 or col2 ~= self.last_col2) and
        self.size.x > 0 then
    ...
    -- update blink timer
    if not config.disable_blink and system.window_has_focus(core.window) and self == core.active_view and
        not self.mouse_selecting then
...
function DocView:draw_line_body(line, x, y)
    ...
    for lidx, line1, col1, line2, col2 in self.doc:get_selections(true) do
        if line >= line1 and line <= line2 then
            local text = self.doc.buffer:get_line(line)
            if line1 ~= line then
                col1 = 1
            end
            if line2 ~= line then
...
function DocView:draw_overlay()
    if core.active_view == self then
        ...
        for _, line1, col1, line2, col2 in self.doc:get_selections() do
            if line1 >= minline and line1 <= maxline and system.window_has_focus(core.window) then
                if ime.editing then
