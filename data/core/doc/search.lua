...
function search.find(doc, line, col, text, opt)
    ...
    local start, finish, step = line, doc.buffer:line_count(), 1
    if opt.reverse then
        start, finish, step = line, 1, -1
    end
    for line = start, finish, step do
        local line_text = doc.buffer:get_line(line)
        if opt.no_case and not opt.regex then
    ...
    if opt.wrap then
        ...
        if opt.reverse then
            return search.find(doc, doc.buffer:line_count(), #doc.buffer:get_line(doc.buffer:line_count()), text, opt)
        else
