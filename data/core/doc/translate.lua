...
function translate.previous_block_start(doc, line, col)
    while true do
        ...
        if doc.buffer:get_line(line - 1):find("^%s*$") and not doc.buffer:get_line(line):find("^%s*$") then
            return line, (doc.buffer:get_line(line):find("%S"))
        end
    end
end
...
function translate.next_block_end(doc, line, col)
    while true do
        if line >= doc.buffer:line_count() then
            return doc.buffer:line_count(), 1
        end
        if doc.buffer:get_line(line + 1):find("^%s*$") and not doc.buffer:get_line(line):find("^%s*$") then
            return line + 1, #doc.buffer:get_line(line + 1)
        end
        line = line + 1
    end
end
...
function translate.start_of_indentation(doc, line, col)
    local s, e = doc.buffer:get_line(line):find("^%s*")
...
function translate.end_of_doc(doc, line, col)
    return doc.buffer:line_count(), #doc.buffer:get_line(doc.buffer:line_count())
end
