...
local function save_session()
    ...
    if fp then
        fp:write("return {recents=", common.serialize(core.recent_projects), ", window=",
            common.serialize(table.pack(system.get_window_size(core.window))), ", window_mode=",
            common.serialize(system.get_window_mode(core.window)), ", previous_find=",
            common.serialize(core.previous_find), ", previous_replace=", common.serialize(core.previous_replace), "}\n")
        fp:close()
    end
end
...
function core.init()
        ...
        for i = 2, #ARGS do
            ...
            if info.type == "dir" then
                ...
                if not ARGS[i]:match("^-psn") then
                    local file_abs = common.is_absolute_path(arg_filename) and arg_filename or
                                         (system.absolute_path(".") .. PATHSEP .. common.normalize_path(arg_filename))
                    if file_abs then
    ...
    ---@type core.titleview
    core.title_view = core.root_view.title_bar
    ...
    cur_node:split("up", core.nag_view, {
        y = true
    })
    cur_node = cur_node.b
    cur_node = cur_node:split("down", core.command_view, {
        y = true
    })
    cur_node = cur_node:split("down", core.status_view, {
        y = true
    })
    ...
    if #plugins_refuse_list.userdir.plugins > 0 or #plugins_refuse_list.datadir.plugins > 0 then
        ...
        for _, entry in pairs(plugins_refuse_list) do
            if #entry.plugins > 0 then
                ...
                msg[#msg + 1] = string.format("Plugins from directory \"%s\":\n%s", common.home_encode(entry.dir),
                    table.concat(msg_list, "\n"))
            end
        end
        core.nag_view:show("Refused Plugins",
            string.format("Some plugins are not loaded due to version mismatch. Expected version %s.\n\n%s.\n\n" ..
...
function core.parse_plugin_details(path, file, mod_version_regex, priority_regex)
    ...
    for line in f:lines() do
        ...
        if not priority then
            ...
            if status and _priority then
                priority = tonumber(_priority)
            end
        end
...
local mod_version_regex = regex.compile([[--.*mod-version:(\d+)(?:\.(\d+))?(?:\.(\d+))?(?:$|\s)]])
...
function core.get_plugin_details(path)
    ...
    if details then
        details.load = require_lua_plugin
    end
    return details
end
...
function core.add_plugins(plugins)
    for i, v in ipairs(plugins) do
        table.insert(core.plugin_list, v)
    end
...
function core.load_plugins()
    ...
    local files, ordered = {}, {{
        ...
        load = load_lua_plugin_if_exists,
        version_match = true,
        file = core.root_project().path .. PATHSEP .. ".lite_project.lua",
        name = "Project Module"
    }}
    ...
    for i = 1, #core.plugin_list do
        ...
        if not config.skip_plugins_version and not plugin.version_match then
            core.log_quiet("Version mismatch for plugin %q[%s] from %s", plugin.name, plugin.version_string,
                common.dirname(plugin.file))
            local rlist = plugin.file:find(USERDIR, 1, true) == 1 and 'userdir' or 'datadir'
        ...
        elseif config.plugins[plugin.name] ~= false then
            ...
            if ok then
                ...
                core.log_quiet("Loaded plugin %q%s from %s in %.1fms", plugin.name, plugin_version,
                    common.dirname(plugin.file), (system.get_time() - start) * 1000)
...
function core.project_for_path(path)
    for i, project in ipairs(core.projects) do
        if project.path:find(path, 1, true) then
            return project
        end
    end
    return nil
...
function core.step()
    ...
    for type, a, b, c, d, e, f in system.poll_event do
        local win_id
        if type == "mousemoved" or type == "mousepressed" then
            win_id = e
        elseif type == "mousereleased" or type == "textediting" or type == "touchpressed" or type == "touchreleased" or
            type == "filedropped" then
            win_id = d
        elseif type == "mousewheel" or type == "resized" then
            win_id = c
        elseif type == "keypressed" or type == "keyreleased" or type == "textinput" then
            win_id = b
        elseif type == "exposed" or type == "minimized" or type == "maximized" or type == "restored" or type ==
            "mouseleft" or type == "focuslost" then
            win_id = a
        elseif type == "touchmoved" then
            win_id = f
        end

        if not win_id or win_id == core.window:get_id() then
            if type == "textinput" and did_keymap then
                did_keymap = false
            elseif type == "mousemoved" then
                core.try(core.on_event, type, a, b, c, d, e, f)
            elseif type == "enteringforeground" then
                ...
                local _, res = core.try(core.on_event, type, a, b, c, d, e, f)
...
function core.deprecation_log(kind)
    if alerted_deprecations[kind] then
        return
    end
