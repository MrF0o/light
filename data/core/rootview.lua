...
function RootView.on_view_mouse_pressed(button, x, y, clicks) end
...
function RootView:on_mouse_pressed(button, x, y, clicks)
  ...
  if self.title_bar:on_mouse_pressed(button, x, y, clicks) then
...
function RootView:on_mouse_released(button, x, y, ...)
  ...
  self.title_bar:on_mouse_released(button, x, y, ...)
    ...
    if button == "left" then
      if self.dragged_node.dragging then
        ...
        if
          ...
          and (node ~= dragged_node or #node.views > 1)
        then
          ...
          elseif split_type == "tab" then -- move besides other tabs
            local tab_index =
              node:get_drag_overlay_tab_position(self.mouse.x, self.mouse.y, dragged_node, self.dragged_node.idx)
            dragged_node:remove_view(self.root_node, view)
...
function RootView:on_mouse_moved(x, y, dx, dy)
  ...
  if dn and not dn.dragging then
    ...
    dn.dragging = common.distance(x, y, dn.drag_start_x, dn.drag_start_y) > style.tab_width * 0.05
    if dn.dragging then
  ...
  if self.title_bar:on_mouse_moved(x, y, dx, dy) then
    return true
  end
  ...
  if not self.overlapping_view then
    return
  end
...
function RootView:update()
  self.title_bar.size.x = self.size.x
  self.title_bar:update()

  self.root_node.position.x = self.position.x
  self.root_node.position.y = self.position.y + self.title_bar.size.y
  self.root_node.size.x = self.size.x
  self.root_node.size.y = self.size.y - self.title_bar.size.y
...
function RootView:update_drag_overlay()
  ...
  if over and not over.locked then
    ...
    if split_type == "tab" and (over ~= self.dragged_node.node or #over.views > 1) then
      ...
      self:set_drag_overlay(
        self.drag_overlay_tab,
        tab_x + (tab_index and 0 or tab_w),
        tab_y,
        style.caret_width,
        tab_h,
