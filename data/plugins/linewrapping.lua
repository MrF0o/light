...
config.plugins.linewrapping = common.merge({
    ...
    config_spec = {
        name = "Line Wrapping",
        {
            ...
            type = "selection",
            default = "letter",
            values = {{"Letters", "letter"}, {"Words", "word"}}
        },
        {
            label = "Guide",
...
local function spew_tokens(doc, line)
    if line < math.huge then
        return math.huge, "normal", doc.buffer:get_line(line)
    end
end
...
function LineWrapping.compute_line_breaks(doc, default_font, line, width, mode)
    ...
    for idx, type, text in get_tokens(doc, line) do
        ...
        if idx == 1 or idx == math.huge and config.plugins.linewrapping.indent then
            ...
            if indent_end then
                begin_width = font:get_width(text:sub(1, indent_end))
            end
        end
...
function LineWrapping.reconstruct_breaks(docview, default_font, width, line_offset)
    if width ~= math.huge then
        ...
        for i = line_offset or 1, doc.buffer:line_count() do
            local breaks, offset = LineWrapping.compute_line_breaks(doc, default_font, i, width,
...
function LineWrapping.update_breaks(docview, old_line1, old_line2, net_lines)
    ...
    for line = new_line1, new_line2 do
        local breaks, begin_width = LineWrapping.compute_line_breaks(docview.doc, docview.wrapped_settings.font, line,
            docview.wrapped_settings.width, config.plugins.linewrapping.mode)
...
function LineWrapping.update_docview_breaks(docview)
    ...
    local width = (type(config.plugins.linewrapping.width_override) == "function" and
                      config.plugins.linewrapping.width_override(docview)) or config.plugins.linewrapping.width_override or
                      (docview.size.x - docview:get_gutter_width() - w)
...
local function get_idx_line_col(docview, idx)
    ...
    if not docview.wrapped_settings then
        if idx > doc.buffer:line_count() then
            return doc.buffer:line_count(), #doc.buffer:get_line(doc.buffer:line_count()) + 1
        end
        return idx, 1
    end
    if idx < 1 then
...
local function get_idx_line_length(docview, idx)
    ...
    if not docview.wrapped_settings then
        if idx > doc.buffer:line_count() then
            return #doc.buffer:get_line(doc.buffer:line_count()) + 1
        end
        return #doc.buffer:get_line(idx)
    end
    local offset = (idx - 1) * 2 + 1
    ...
    if docview.wrapped_lines[offset + 2] and docview.wrapped_lines[offset + 2] == docview.wrapped_lines[offset] then
        ...
        return #doc.buffer:get_line(docview.wrapped_lines[offset]) - docview.wrapped_lines[offset + 1] + 1
    end
end

local function get_total_wrapped_lines(docview)
    if not docview.wrapped_settings then
        return docview.doc and docview.doc.buffer:line_count()
    end
...
local function get_line_idx_col_count(docview, line, col, line_end, ndoc)
    local doc = docview.doc
    if not docview.wrapped_settings then
        return common.clamp(line, 1, doc.buffer:line_count()), col, 1, 1
    end
    if line > doc.buffer:line_count() then
        return get_line_idx_col_count(docview, doc.buffer:line_count(),
            #doc.buffer:get_line(doc.buffer:line_count()) + 1)
    end
    ...
    local count = (docview.wrapped_line_to_idx[line + 1] or (get_total_wrapped_lines(docview) + 1)) -
                      (docview.wrapped_line_to_idx[line] or get_total_wrapped_lines(docview))
...
local function get_line_col_from_index_and_x(docview, idx, x)
    ...
    return line, #doc.buffer:get_line(line)
end

local open_files = setmetatable({}, {
    __mode = "k"
})

local old_doc_insert = Doc.raw_insert
function Doc:raw_insert(line, col, text, undo_stack, time)
    local old_lines = self.buffer:line_count()
    ...
    if open_files[self] then
        for i, docview in ipairs(open_files[self]) do
            if docview.wrapped_settings then
                local lines = self.buffer:line_count() - old_lines
...
function DocView:get_x_offset_col(line, x)
    if not self.wrapped_settings then
        return old_get_x_offset_col(self, line, x)
    end
...
function DocView:get_col_x_offset(line, col, line_end)
    if not self.wrapped_settings then
        return old_get_col_x_offset(self, line, col)
    end
    ...
    for _, type, text in self.doc.highlighter:each_token(line) do
        ...
        while text ~= nil and token_offset <= #text do
            ...
            if next_line ~= line then
                next_line_start_col = #self.doc.buffer:get_line(line)
            end
            ...
            if total_offset ~= next_line_start_col or max_length == 0 then
                break
            end
...
function DocView:draw_line_body(line, x, y)
    ...
    for lidx, line1, col1, line2, col2 in self.doc:get_selections(true) do
        if line >= line1 and line <= line2 then
            ...
            if line2 ~= line then
                col2 = #self.doc.buffer:get_line(line) + 1
            end
            if col1 ~= col2 then
    ...
    for lidx, line1, col1, line2, col2 in self.doc:get_selections(true) do
        ...
        if draw_highlight ~= false and config.highlight_current_line and line1 == line and core.active_view == self then
...
function translate.end_of_line(doc, line, col)
    if not core.active_view or core.active_view.doc ~= doc or not core.active_view.wrapped_settings then
        old_translate_end_of_line(doc, line, col)
    end
...
function translate.start_of_line(doc, line, col)
    if not core.active_view or core.active_view.doc ~= doc or not core.active_view.wrapped_settings then
        old_translate_start_of_line(doc, line, col)
    end
    local idx, ncol = get_line_idx_col_count(core.active_view, line, col)
    local nline, ncol2 = get_idx_line_col(core.active_view, idx - 1)
    if nline ~= line then
