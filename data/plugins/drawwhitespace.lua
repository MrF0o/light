...
local function reset_cache_if_needed()
    local settings = config.plugins.drawwhitespace
    if not ws_cache or cached_settings.show_leading ~= settings.show_leading or cached_settings.show_trailing ~=
        settings.show_trailing or cached_settings.show_middle ~= settings.show_middle or cached_settings.show_middle_min ~=
        settings.show_middle_min or cached_settings.color ~= settings.color or cached_settings.leading_color ~=
        settings.leading_color or cached_settings.middle_color ~= settings.middle_color or
        cached_settings.trailing_color ~= settings.trailing_color -- we assume that the entire table changes
    or cached_settings.substitutions ~= settings.substitutions then
        reset_cache()
    end
end
...
function Highlighter:insert_notify(line, n, ...)
    ...
    local to = math.min(line + n, self.doc.buffer:line_count())
    for i = self.doc.buffer:line_count() + n, to, -1 do
...
function Highlighter:remove_notify(line, n, ...)
    ...
    local to = math.max(line + n, self.doc.buffer:line_count())
    for i = line, to do
        ws_cache[self][i] = ws_cache[self][i + n]
    end
end
...
function DocView:draw_line_text(idx, x, y)
    if not config.plugins.drawwhitespace.enabled or getmetatable(self) ~= DocView then
    ...
    if not ws_cache[self.doc.highlighter] or ws_cache[self.doc.highlighter].font ~= font or
        ws_cache[self.doc.highlighter].font_size ~= font_size or ws_cache[self.doc.highlighter].indent_size ~=
        indent_size then
        ws_cache[self.doc.highlighter] = setmetatable({
            font = font,
            font_size = font_size,
    ...
    if not ws_cache[self.doc.highlighter][idx] then -- need to cache line
        ...
        local tx
        local text = self.doc.buffer:get_line(idx)
    ...
    for i = 1, #cache, 4 do
        ...
        if #partials == 0 and not config.plugins.drawwhitespace.show_selected_only then
            ...
            for _, p in pairs(partials) do
                if p then
                    core.push_clip_rect(table.unpack(p))
                end
                renderer.draw_text(font, sub, tx, ty, color)
                if p then
                    core.pop_clip_rect()
