...
config.plugins.autocomplete = common.merge({
    ...
    config_spec = {
        ...
        },
        {
            ...
            type = "selection",
            default = "global",
            values = {{"All Documents", "global"}, {"Current Document", "local"}, {"Related Documents", "related"},
                      {"Known Symbols", "none"}},
...
function autocomplete.add(t, manually_triggered)
    ...
    if not manually_triggered then
        autocomplete.map[t.name] = {
            files = t.files or ".*",
            items = items
        }
    else
        autocomplete.map_manually[t.name] = {
            files = t.files or ".*",
            items = items
        }
    end
...
core.add_thread(function()
    ...
    local function get_symbols(doc)
        ...
        if doc.disable_symbols then
            return s
        end
        local i = 1
        local symbols_count = 0
        while i <= doc.buffer:line_count() do
            for sym in doc.buffer:get_line(i):gmatch(config.symbol_pattern) do
...
local function get_suggestions_rect(av)
    ...
    for i = start_index, start_index + show_count - 1 do
        ...
        if not hide_icons and icon and autocomplete.icons[icon] then
            local icon_width = autocomplete.icons[icon].font:get_width(autocomplete.icons[icon].char)
    ...
    return x, y - style.padding.y, max_width, max_items * (th + style.padding.y) + style.padding.y, has_icons
end
    ...
    if #line > max_chars then
        ...
        for char in line:gmatch(".") do
            ...
            if #new_line < max_chars then
                ...
                if not prev_char:match("%s") and not string.sub(line, position + 1, 1):match("%s") and position <
                    line_len then
                    new_line = new_line .. "-"
                end
                table.insert(lines, new_line)
...
local desc_font = style.code_font:copy(config.plugins.autocomplete.desc_font_size * SCALE)
local function draw_description_box(text, av, sx, sy, sw, sh)
    ...
    if sx - av.position.x < av.size.x - (sx - av.position.x) - sw then
        ...
        draw_left = true;
        max_chars = ((sx - av.position.x - (style.padding.x / 4) - style.scrollbar_size) / char_width) - 5
    end
...
local function draw_suggestions_box(av)
    ...
    for i = start_index, start_index + show_count - 1, 1 do
        ...
        local x_adv = common.draw_text(font, color, s.text, "left", rx + icon_l_padding + style.padding.x, y, rw, lh)
        core.pop_clip_rect()
        ...
        if s.info and not hide_info then
            ...
            common.draw_text(style.font, color, s.info, "right", rx, y, rw - icon_r_padding - style.padding.x, lh)
        end
        y = y + lh
    ...
    common.draw_text(style.font, style.accent, tostring(suggestions_idx) .. "/" .. tostring(#suggestions), "right", rx,
        y, rw - style.padding.x, lh)
end
...
function autocomplete.add_icon(name, character, font, color)
    ...
    assert(not color or color_type == "table" or (color_type == "string" and style.syntax[color]),
    ...
    ["autocomplete:complete"] = function(dv)
        ...
        if not inserted then
            ...
            for _, line1, col1, line2, _ in doc:get_selections(true) do
                local n = col1 - 1
                local line = doc.buffer:get_line(line1)
                for i = 1, sz + 1 do
                    local j = sz - i
