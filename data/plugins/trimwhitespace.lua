...
function trimwhitespace.trim(doc)
    ...
    for i = 1, doc.buffer:line_count() do
...
function trimwhitespace.trim_empty_end_lines(doc, raw_remove)
    for _ = doc.buffer:line_count(), 1, -1 do
        local l = doc.buffer:line_count()
        if l > 1 and doc.buffer:get_line(l) == "\n" then
