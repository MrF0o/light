...
function TreeView:get_cached(project, path)
    ...
    if not t then
        if not self.watches[project] then
            self.watches[project] = Dirwatch.new()
        end
    ...
    if t.expanded and t.type == "dir" and not t.files then
        ...
        table.sort(t.files, function(a, b)
            return system.path_compare(a.name, a.type, b.name, b.type)
        end)
    end
    return t
end
        ...
        for i, file in ipairs(dir.files) do
            if self.show_hidden or not file.name:find("^%.") then
                count_lines = count_lines +
                                  self:get_items(project, path .. PATHSEP .. file.name, x, y + count_lines * h, w, h)
            end
        end
    end
    return count_lines
...
function TreeView:each_item()
    return coroutine.wrap(function()
        ...
        for k, project in ipairs(core.projects) do
            count_lines = count_lines +
                              self:get_items(project, project.path, ox, oy + style.padding.y + h * count_lines,
                    self.size.x, h)
        end
...
function TreeView:set_selection(selection, selection_y, center, instant)
    ...
    if selection and selection_y and (selection_y <= 0 or selection_y >= self.size.y) then
...
function TreeView:update()
    ...
    if self.size.x == 0 or self.size.y == 0 or not self.visible then
        return
    end
    ...
    if config.highlight_focused_file then
        ...
        if current_node and not current_node.locked and current_active_view ~= self and current_active_view ~=
            self.last_active_view then
            self.selected_item = nil
            self.last_active_view = current_active_view
            if DocView:is_extended_by(current_active_view) then
                local abs_filename = current_active_view.doc and current_active_view.doc.abs_filename or ""
                self:set_selection_to_path(abs_filename, config.expand_dirs_to_focused_file,
                    config.scroll_to_focused_file, not config.animate_scroll_to_focused_file)
            end
        end
    end
...
view.node = node:split("left", view, {
    x = true
}, true)
...
if config.plugins.toolbarview ~= false and toolbar_plugin then
    ...
    view.node:split("up", toolbar_view, {
        y = true
    })
...
function TreeView:on_context_menu()
    return {
        items = {{
            text = "Open in System",
            command = "treeview:open-in-system"
        }, ContextMenu.DIVIDER, {
            text = "Rename",
            command = "treeview:rename"
        }, {
            text = "Delete",
            command = "treeview:delete"
        }, {
            text = "New File",
            command = "treeview:new-file"
        }, {
            text = "New Folder",
            command = "treeview:new-folder"
        }, {
            text = "Remove directory",
            command = "treeview:remove-project-directory"
        }, {
            text = "Find in Directory",
            command = "treeview:search-in-directory"
        }}
    }, self
end
...
command.add(function()
    ...
    ["treeview:delete"] = function(item)
        ...
        core.nag_view:show(string.format("Delete %s", file_type), string.format(
            "Are you sure you want to delete the %s?\n%s: %s", file_type:lower(), file_type, relfilename), opt,
            function(item)
...
command.add(function()
    local item = treeitem()
    return item and not is_primary_project_folder(item.abs_filename) and is_project_folder(item), item
end, {
...
config.plugins.treeview.config_spec = {
    name = "Treeview",
    {
        ...
        path = "size",
        type = "number",
        default = toolbar_view and math.ceil(toolbar_view:get_min_width() / SCALE) or 200 * SCALE,
        min = toolbar_view and toolbar_view:get_min_width() / SCALE or 200 * SCALE,
        ...
        on_apply = function(value)
            view:set_target_size("x", math.max(value, toolbar_view and toolbar_view:get_min_width() or 200 * SCALE))
        end
    },
    {
