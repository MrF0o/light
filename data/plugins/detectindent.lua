    ...
    for x = 1, count do
        ...
        for y = 1, count do
            if y ~= x and stat[y] % indent == 0 then
                score = score + 1
            elseif indent > stat[y] and (indent_occurrences_more_than_once(stat, y) or (y == count and stat[y] > 1)) then
                score = 0
                break
            end
        end
...
local function get_comment_patterns(syntax, _loop)
    ...
    for idx = 1, #syntax.patterns do
        ...
        local startp = ""
        if type(pattern.type) == "string" and (pattern.type == "comment" or pattern.type == "string") then
            local not_is_string = pattern.type ~= "string"
            if pattern.pattern then
                startp = type(pattern.pattern) == "table" and pattern.pattern[1] or pattern.pattern
            ...
            elseif pattern.regex then
                startp = type(pattern.regex) == "table" and pattern.regex[1] or pattern.regex
                if not_is_string and startp:sub(1, 1) ~= "^" then
                    startp = "^\\s*" .. startp
                elseif not_is_string then
                    startp = "^\\s*" .. startp:sub(2, startp:len())
                ...
                if type(pattern.regex) == "table" then
                    table.insert(comments, {
                        "r",
                        regex.compile(startp),
                        regex.compile(pattern.regex[2]),
                        r = startp
                    })
                elseif not_is_string then
                    table.insert(comments, {
        ...
        elseif pattern.syntax then
            local subsyntax = type(pattern.syntax) == 'table' and pattern.syntax or
                                  core_syntax.get("file" .. pattern.syntax, "")
    ...
    if #comments == 0 then
        local single_line_comment = syntax.comment and escape_comment_tokens(syntax.comment) or nil
        local block_comment = nil
        if syntax.block_comment then
            block_comment = {escape_comment_tokens(syntax.block_comment[1]),
    ...
    table.sort(comments, function(c1, c2)
        ...
        if (c1[1] == "p" and string.find(c1[2], "^%s*", 1, true)) or
            (c1[1] == "r" and string.find(c1["r"], "^\\s*", 1, true)) then
            comment1 = true
        end
        if (c2[1] == "p" and string.find(c2[2], "^%s*", 1, true)) or
            (c2[1] == "r" and string.find(c2["r"], "^\\s*", 1, true)) then
            comment2 = true
        end
...
local function get_non_empty_lines(syntax, doc)
    return coroutine.wrap(function()
        ...
        local count = doc.buffer:line_count()
        for idx = 1, count do
            local line = doc.buffer:get_line(idx)
            if line:gsub("^%s+", "") ~= "" then
                    ...
                    if not inside_comment then
                        for c = 1, #comments do
                            ...
                            if comment[1] == "p" then
                                ...
                                if comment[3] then
                                    local start, ending = regex.find_offsets(comment[2], line, 1, regex.ANCHORED)
                                    if start then
                                        if not regex.find_offsets(comment[3], line, ending + 1, regex.ANCHORED) then
                                            is_comment = true
                                            inside_comment = true
                                            end_regex = comment[3]
                                        end
                ...
                if not is_comment and not inside_comment then
                    i = i + 1
...
local function detect_indent_stat(doc)
    ...
    for i, text in get_non_empty_lines(doc.syntax, doc) do
        local spaces = text:match("^ +")
        if spaces and #spaces > 1 then
            table.insert(stat, #spaces)
        end
...
local function update_cache(doc)
    ...
    cache[doc] = {
        type = type,
        size = size,
        confirmed = (score >= score_threshold)
    }
...
command.add(function()
    return core.active_view:is(DocView) and cache[core.active_view.doc] and cache[core.active_view.doc].type == "soft"
end, {
...
command.add(function()
    return core.active_view:is(DocView) and cache[core.active_view.doc] and cache[core.active_view.doc].type == "hard"
end, {
