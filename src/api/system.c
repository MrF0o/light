...
top:
  ...
  switch (e.type) {
    ...
    case SDL_EVENT_WINDOW_EXPOSED:
      {
        RenWindow* window_renderer = ren_find_window_from_id(e.window.windowID);
        rencache_invalidate(window_renderer);
    ...
    case SDL_EVENT_DID_ENTER_FOREGROUND:
      {
          ...
          while (window_count) {
            rencache_invalidate(window_list[--window_count]);
          }
        #else
          RenWindow** window_list;
          size_t window_count = ren_get_window_list(&window_list);
...
static int f_ftruncate(lua_State *L) {
  ...
  // LuaJIT uses a simpler FILE* wrapper without luaL_Stream
...
static void* api_require(const char* symbol) {
  static const lua_function_node nodes[] = {
    #ifdef LUAJIT_VERSION
    // LuaJIT 2.1 is compatible with Lua 5.1 + some 5.2 features
    ...
    U(optnumber), U(pushresult), U(ref), U(unref), U(where), U(prepbuffer),
    U(loadbuffer), U(loadfile), U(register), U(typerror), U(findtable),
    P(atpanic), P(checkstack), P(close), P(concat), P(createtable), P(dump),
    P(error), P(gc), P(getallocf), P(getfield), P(gethook), P(gethookcount),
    ...
    P(typename), P(xmove), P(newuserdata), P(call), P(equal), P(getfenv),
    P(lessthan), P(objlen), P(pcall), P(setfenv), P(tointeger),
    P(tonumber), P(yield), P(cpcall),
    S(luaopen_base), S(luaopen_debug), S(luaopen_io), S(luaopen_math),
    S(luaopen_os), S(luaopen_package), S(luaopen_string), S(luaopen_table),
    S(luaopen_jit), S(api_load_libs),
    #else
