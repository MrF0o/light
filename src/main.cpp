...
// Global Clay renderer - initialized once at startup
namespace clay {
    extern ClayRenderer* g_clay_renderer;
}
...
init_lua:
  ...
  const char *init_lite_code =
      ...
      "  local start_path = (MACOS_RESOURCES or (prefix and prefix .. '/share/light' or "
      "exedir .. '/data')) .. '/core/start.lua'\n"
      "  local f = io.open(start_path, 'r')\n"
      "  if f then\n"
      "    f:close()\n"
      "  else\n"
      "    start_path = '" LITE_BUILD_DIR "' .. '/start.lua'\n"
      "  end\n"
      "  dofile(start_path)\n"
  ...
  if (clay::g_clay_renderer) {
    delete clay::g_clay_renderer;
    clay::g_clay_renderer = nullptr;
  }
