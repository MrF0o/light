...
Clay_ElementId Clay__AttachId(Clay_ElementId elementId) {
  ...
  Clay__StringArray_Set(&context->layoutElementIdStrings,
                        context->layoutElements.length - 1, elementId.stringId);
  return elementId;
}
      ...
      CLAY({.layout = {.sizing = {CLAY_SIZING_GROW(0), CLAY_SIZING_FIXED(300)},
        ...
        for (int32_t elementConfigIndex = 0;
          ...
          case CLAY__ELEMENT_CONFIG_TYPE_ASPECT: {
            ...
            CLAY({.id = CLAY_ID("Clay__DebugViewElementInfoAspectRatioBody"),
    ...
    if (scrollContainerData->elementId == id.id) {
      Clay_ClipElementConfig *clipElementConfig =
          Clay__FindElementConfigWithType(scrollContainerData->layoutElement,
                                          CLAY__ELEMENT_CONFIG_TYPE_CLIP)
              .clipElementConfig;
      if (!clipElementConfig) { // This can happen on the first frame before a
